...
void sensor_callback(const sensor_msgs::Imu &imu) {//const geometry_msgs::Point32 &point) {
  message = imu;
...
int main( int argc, char** argv )
{
  ...
  ros::Subscriber sensor_pos_sub = n.subscribe("imu", 100, sensor_callback);
  ...
  while (ros::ok()) // ros::ok()
  {
    ...
    ROS_INFO("%f", message.orientation.w);
